...
def generate_allure_report(**kwargs):
    ...
    if os.path.exists(allure_results_dir):
        ...
        if os.listdir(allure_results_dir):
            pass
            # print(f"{allure_results_dir} - allure测试结果集目录正确生成，可以生成allure html报告！")
        else:
            print(f"{allure_results_dir} - allure测试结果集目录未正确生成，，无法生成allure html报告！")
    else:
        print(f"{allure_results_dir} - allure测试结果集目录未生成，无法生成allure html报告！")
    ...
    try:
        ...
        # print(
        #     f"执行命令[{PlatformHandle().allure} generate {allure_results_dir} -o {allure_report_dir} --clean]：{result.stdout}")  # 正常日志
        # print(result.stdout)  # 正常日志
        if result.stderr:
            print(f"⚠️ Allure 生成报告时有警告/错误：{result.stderr}")
