...
MOCK_CONFIG = {
    "enabled": True,
    "mode": "disabled",
    "recordings_dir": os.path.join(os.path.dirname(os.path.dirname(__file__)), "outputs", "mock_recordings"),
    "auto_save": True,
    "default_delay": 0.0,
}
...
# ------------------------------------ 邮件通知内容 ----------------------------------------------------#
email_subject = f"API 接口自动化测试报告"
email_content = """
           各位同事, 大家好:
           接口自动化用例于 <strong>${start_time} </strong> 开始运行，运行时长：<strong>${run_time} s</strong>， 目前已执行完成。
           ---------------------------------------------------------------------------------------------------------------
...
# ------------------------------------ 钉钉通知内容 ----------------------------------------------------#
ding_talk_title = f"API 接口自动化测试报告"
ding_talk_content = """
           各位同事, 大家好:
           ### 接口自动化用例于 ${start_time} 开始运行，运行时长：${run_time} s， 目前已执行完成。
            ---------------------------------------------------------------------------------------------------------------
           #### 测试人： ${tester}
           #### 所属部门： ${department}
           #### 项目环境： ${env} 
